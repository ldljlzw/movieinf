...
public:
  ...
  double Eps;
public:
  TCascade() : NIdHitH(), CurProb(0), Alpha(1.0), Eps(EPS), Model(0) { }
  TCascade(const double &alpha) : NIdHitH(), CurProb(0), Eps(EPS), Model(0) { Alpha = alpha; }
  TCascade(const double &alpha, const int &model) : NIdHitH(), CurProb(0), Eps(EPS) { Alpha = alpha; Model = model; }
...
public:
  ...
  void GenCascade(TCascade& C, const int& TModel, const double &window, TIntPrIntH& EdgesUsed,
  ...
  void GenNoisyCascade(TCascade& Cascade, const int& TModel, const double& window,
		  	  	  	   TIntPrIntH& EdgesUsed, const double& std_waiting_time=0, const double& std_beta=0,

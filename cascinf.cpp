...
void TNetInfBs::LoadGroundTruthTxt(TSIn& SIn) {
	...
	while (!SIn.Eof()) {
		...
		Alphas.AddDat(TIntPr(NIdV[0].GetInt(), NIdV[1].GetInt())) = NIdV[1].GetFlt();
	}
...
void TNetInfBs::GenCascade(TCascade& C, const int& TModel, const double &window, TIntPrIntH& EdgesUsed,
		...
		while (true) {
			...
			for (int e = 0; e < NI.GetOutDeg(); e++) {
				...
				case 0:
					...
					sigmaT = alpha*TInt::Rnd.GetExpDev();
					break;
				case 1:
...
void TNetInfBs::GenNoisyCascade(TCascade& C, const int& TModel, const double &window, TIntPrIntH& EdgesUsed,
	...
	GenCascade(C, TModel, window, EdgesUsedC, std_waiting_time, std_beta);
